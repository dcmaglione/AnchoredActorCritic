...
fine_tune_naive() {
    ...
    PYTHONUNBUFFERED=1 python -m envs.LunarLander.fine_tune_landers "$file" --seed 1 --initial-random 500.0 --epochs 10 | 
...
fine_tune_anchored() {
    ...
    PYTHONUNBUFFERED=1 python -m envs.LunarLander.fine_tune_landers "$file" --seed 1 -a --initial-random 500.0 --epochs 10 | 
    tee >(grep "saving at" | tail -n 1 | sed 's/.*saving at //' >> "$output_file")
}
