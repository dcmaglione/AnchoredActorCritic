...
fine_tune_naive() {
    ...
    python -m envs.Reacher.train_reacher -s_s 5000 -s 1 -e 15 -d 0.1 -a_n 0.01 -p "$file" | 
    grep "saving at" | tail -n 1 | sed 's/.*saving at //' >> "$output_file"
}
...
fine_tune_anchored() {
    ...
    python -m envs.Reacher.train_reacher -a -s_s 5000 -s 1 -e 15 -d 0.1 -a_n 0.01 -p "$file" | 
    grep "saving at" | tail -n 1 | sed 's/.*saving at //' >> "$output_file"
}
