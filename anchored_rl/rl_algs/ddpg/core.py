...
def mlp_actor_critic(obs_space: spaces.Box, act_space: spaces.Box, obs_normalizer=None, actor_hidden_sizes=(32,32), critic_hidden_sizes=(256,256)) -> tuple[Model, Model]:
    if obs_normalizer is None:
        obs_normalizer = obs_space.high*0.0 + 1.0
    obs_normalizer = np.array(obs_normalizer)
    return ( actor(obs_space, act_space, actor_hidden_sizes, obs_normalizer)
